...
module app {
    ...
    export function luckyYouApiUrl(path?: string) { return '/api/lucky_you/' + (path || ''); };
    export function skypeRecordingsApiUrl(path?: string) { return '/api/skype_recordings/' + (path || ''); };
    export function storytellingApiUrl(path?: string) { return '/api/storytelling/' + (path || ''); };
    ...
    export function captureSupported(mimeType: string) {
    ...
    /** Rounds the value to the specified number of digits in the fractional part. */
    export function roundTo(val: number, fractionDigits: number) {
    ...
    export function formatMoney(val: any): string {
        var num = app.numberToMoney(val);
        return num == null ? '' : '' + num;
    };

    export function numberToMoney(val: any): number {
        return isNumber(val) ? Number(app.roundTo(val, 2).toFixed(2)) : null;
    };
    ...
    /** Find the first array element satisfying the test. Pure JavaScript. IE9+ */
    ...
    export function arrGroupBy<T>(arr: T[], keySelector: (item: T) => any) {
        ...
        return Object.keys(groups).map((i) => { return <T[]>groups[i]; });
    };
    ...
    export function getBlobUrl(containerName: string, blobName: string) {
        if (containerName && blobName) {
            ...
            return app.isDevHost()
                ? protocol + '//' + getHostname() + '/api/exercises/blob?container=' + containerName + '&name=' + blobName
    ...
    // Sounds are not played on mobile devices without a user gesture.
