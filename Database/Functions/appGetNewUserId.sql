...
CREATE FUNCTION [dbo].[appGetNewUserId]() 
RETURNS int
AS
BEGIN
/*
...
We generate random Ids and test them against dbo.aspnetUsers until we find a vacant one.
*/
...
with MyCTE (Id, Attempt) as (
	...
	from MyCTE MC
	where exists (select * from dbo.aspnetUsers where Id = MC.Id)
)
select top(1) @Id = Id 
from MyCTE MC
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[appGetNewUserId] TO [websiterole]
    AS [dbo];
