CREATE FUNCTION [dbo].[friGetRandomTeachers]
(
	@ViewSession int, -- Should be the same for requests from the same page to ensure continuity of pagin/scrolling. Usually a value may be get in JS as Date.now() % 1000000.
	@Bucket int
)
/*
...
*/
	insert @t (Id, DisplayName, Announcement)
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[friGetRandomTeachers] TO [websiterole]
    AS [dbo];
