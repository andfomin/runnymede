...
CREATE FUNCTION [dbo].[libGetNewResourceId]() 
RETURNS int
AS
BEGIN
/*
...
We generate random Ids and test them against dbo.libResources until we find a vacant one.
*/
...
with MyCTE (Id, Attempt) as (
	...
	from MyCTE MC
	where exists (select * from dbo.libResources where Id = MC.Id)
)
select top(1) @Id = Id 
from MyCTE MC
