...
CREATE PROCEDURE [dbo].[relAddLearnerToTutor]
	@LearnerUserId int,
	@TutorUserId int
AS
BEGIN
...
begin try
	...
	if not exists (select * from dbo.relLearnersTutors where LearnerUserId = @LearnerUserId and TutorUserId = @TutorUserId) begin
		...
		select @LearnerDisplayName = DisplayName from dbo.appUsers where Id = @LearnerUserId and IsTutor = 0;
	
		if @LearnerDisplayName is null 
			raiserror('%s,%d: The user is supposed to be not a tutor.', 16, 1, @ProcName, @LearnerUserId);
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[relAddLearnerToTutor] TO [websiterole]
    AS [dbo];
